"""
ARMA Model Runner
Fits ARMA models using best parameters from gridsearch and generates forecasts + metrics.
"""
...
def run_arma_models(config_path="config.yaml"):
    """
    Run ARMA models using best parameters from gridsearch.
    ...
    best_params_path = metrics_dir / "arma_best_models.csv"

    if not best_params_path.exists():
        print(f"ERROR: {best_params_path} not found. Run ARMA gridsearch first!")
        return
    ...
    for csv_path in cfg["cleaned_files"]:
        ...
        dev = y_diff.iloc[split_dev:split_test]
        test = y_diff.iloc[split_test:]
        ...
        steps = list(range(max(warm_up, 1), len(y_diff) - horizon, stride))
        print(f"  Running {len(steps)} forecasting steps...")

        forecasts = []
        for t in tqdm(steps, desc=f"{country} ARMA"):
            train_end = y_diff.index[t - 1]

            fcast = result.get_forecast(steps=horizon, dynamic=False)
                ...
                forecasts.append(
                    {
                        ...
                        "model_type": "ARMA",
                        "y_true": y_diff.loc[fc_timestamp],
        ...
        fc_dev.to_csv(forecasts_dir / f"{country}_ARMA_forecasts_dev.csv", index=False)
        fc_test.to_csv(
            forecasts_dir / f"{country}_ARMA_forecasts_test.csv", index=False
        )
        ...
        pd.DataFrame(metrics).to_csv(
            metrics_dir / f"{country}_ARMA_metrics.csv", index=False
        )

        all_metrics.extend(metrics)
        print(f"  ✓ {country} ARMA complete")

    pd.DataFrame(all_metrics).to_csv(
        metrics_dir / "ARMA_metrics_summary.csv", index=False
    )
