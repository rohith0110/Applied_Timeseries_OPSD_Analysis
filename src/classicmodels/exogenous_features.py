...
def build_future_exogenous(
    ...
    if include_wind:
        if (
            ...
            and "wind" in reference_df.columns
            and not reference_df["wind"].dropna().empty
        ):
            last_wind = reference_df["wind"].dropna().iloc[-1]
        else:
            last_wind = 0.0
        parts.append(
            pd.DataFrame({"wind": np.repeat(last_wind, periods)}, index=future_index)
        )

    if include_solar:
        if (
            reference_df is not None
            and "solar" in reference_df.columns
            and not reference_df["solar"].dropna().empty
        ):
            last_solar = reference_df["solar"].dropna().iloc[-1]
        else:
            last_solar = 0.0
        parts.append(
            pd.DataFrame({"solar": np.repeat(last_solar, periods)}, index=future_index)
        )
